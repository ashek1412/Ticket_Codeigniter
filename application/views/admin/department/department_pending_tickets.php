...
?>
	<div class="content">
		<div class="page-title-cont clearfix">
			<h3>Open Tickets - "<?php echo $dpt_info->name; ?>" Department</h3>
		</div>
			...
			<div class="col margin-top col-sm-12">
				<div class="cont clearfix">
					<div class="head clearfix">
						<h4 class="pull-left">Tickets awaiting client's reply</h4>
						...
						<thead>
							...
							foreach($sorting as $sorted) {
								...
								if($sort == $sorted['c']) {
									...
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/admin/ticket-department/'.$dpt_info->id.'/new-tickets/?sort='.$sorted['c'].'&w='.$direction.$s.'">';
									echo '<i class="fa fa-sort hid"></i>'.$arrow.$sorted['title'];
									echo '</th>';
								}else{
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/admin/ticket-department/'.$dpt_info->id.'/new-tickets/?sort='.$sorted['c'].'&w=d'.$s.'">';
					...
					<div id="pagination">
						...
						if($page > 1) {
							if(isset($_GET['search']))
								echo '<a href="' . $base_url . 'panel/admin/ticket-department/'.$dpt_info->id.'/new-tickets/?page=' . ($page-1) . '&search='.$_GET['search'].$srt.'" class="prev"><i class="fa fa-caret-left"></i></a>';
							else
								echo '<a href="' . $base_url . 'panel/admin/ticket-department/'.$dpt_info->id.'/new-tickets/?page=' . ($page-1) . $srt .'" class="prev"><i class="fa fa-caret-left"></i></a>';
						}
						if($total_pages > $page) {
							if(isset($_GET['search']))
								echo '<a href="' . $base_url . 'panel/admin/ticket-department/'.$dpt_info->id.'/new-tickets/?page=' . ($page+1) . '&search='.$_GET['search'].$srt.'" class="next"><i class="fa fa-caret-right"></i></a>';
							else
								echo '<a href="' . $base_url . 'panel/admin/ticket-department/'.$dpt_info->id.'/new-tickets/?page=' . ($page+1) . $srt .'" class="next"><i class="fa fa-caret-right"></i></a>';
						}
						?>
					</div>
