			...
			<div class="col margin-top col-sm-12">
				<div class="cont clearfix">
					<div class="head clearfix">
						<h4 class="pull-left">List of All Uploaded Desktop ECU Files</h4>
...
<!--								</div>-->
					...
					<table class="table t-departments">
						<thead>
							...
							foreach($sorting as $sorted) {
								...
								if($sort == $sorted['c']) {
									...
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/admin/all-ecu-files/?sort='.$sorted['c'].'&w='.$direction.$s.'">';
									echo '<i class="fa fa-sort hid"></i>'.$arrow.$sorted['title'];
									echo '</th>';
								}else{
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/admin/all-ecu-files/?sort='.$sorted['c'].'&w=d'.$s.'">';
						...
						<tbody>
							<?php
							foreach($all_ecu_files->result() as $row) {
							?>
							<tr data-href="<?php echo $base_url . 'panel/admin/ecu-file/' . $row->id; ?>">
								<td><?php echo $row->desktop_user; ?></td>
                                <td><?php echo $row->cus_dtc; ?></td>
								<td><?php echo $row->ecu_file_type; ?></td>
                                <td><?php echo $row->ecu_file; ?></td>
								<td><?php echo date('M jS, Y \a\t H:i:s', strtotime($row->entry_date)); ?></td>
                                <td>
                                    <?php
                                    if($row->file_stat == '1')
                                        echo '<div class="badge red">Pending</div>';
                                    elseif($row->file_stat == '2')
					...
					<div id="pagination">
						...
						if($page > 1) {
							if(isset($_GET['search']))
								echo '<a href="' . $base_url . 'panel/admin/all-ecu-files/?page=' . ($page-1) . '&search='.$_GET['search'].$srt.'" class="prev"><i class="fa fa-caret-left"></i></a>';
							else
								echo '<a href="' . $base_url . 'panel/admin/all-ecu-files/?page=' . ($page-1) . $srt .'" class="prev"><i class="fa fa-caret-left"></i></a>';
						}
						if($total_pages > $page) {
							if(isset($_GET['search']))
								echo '<a href="' . $base_url . 'panel/admin/all-ecu-files/?page=' . ($page+1) . '&search='.$_GET['search'].$srt.'" class="next"><i class="fa fa-caret-right"></i></a>';
							else
								echo '<a href="' . $base_url . 'panel/admin/all-ecu-files/?page=' . ($page+1) . $srt .'" class="next"><i class="fa fa-caret-right"></i></a>';
						}
						?>
					</div>
					</div>
