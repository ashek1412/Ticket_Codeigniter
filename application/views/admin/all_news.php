			...
			<div class="col margin-top col-sm-12">
				<div class="cont clearfix">
					<div class="head clearfix">
						<h4 class="pull-left">List of all News</h4>
						...
						<div class="pull-right">
							<div class="clearfix">
								...
								<div class="pull-right">
									<button class="btn btn-green besides-search" name="drop" data-drop="new-news" onclick="window.location.href='/magic/panel/admin/new-news'">Add News</button>
								</div>
							</div>
						...
						<thead>
							...
							foreach($sorting as $sorted) {
								...
								if($sort == $sorted['c']) {
									...
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/admin/all-news/?sort='.$sorted['c'].'&w='.$direction.$s.'">';
									echo '<i class="fa fa-sort hid"></i>'.$arrow.$sorted['title'];
									echo '</th>';
								}else{
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/admin/all-news/?sort='.$sorted['c'].'&w=d'.$s.'">';
						...
						<tbody>
							...
							<tr>
                                ...
                                <td>
                                     ...
                                     <?php echo "Client:".$row->email_client."</br> Customer:".$row->email_customer."</br> Email List:".$row->cnt; ?>
								...
								<td>
									<?php
											...
											echo '<a href="'.$base_url.'panel/admin/news/'.$row->id.'/delete" title="Delete news" name="delete-admin"><i class="fa fa-close"></i></a>';

									?>
									<a href="<?php echo $base_url; ?>panel/admin/news/<?php echo $row->id; ?>/edit" title="Edit news" name="edit-news"><i class="fa fa-pencil"></i></a>
								</td>
							</tr>
							<?php
							}
					...
					<div id="pagination">
						...
						if($page > 1) {
							if(isset($_GET['search']))
								echo '<a href="' . $base_url . 'panel/admin/all-news/?page=' . ($page-1) . '&search='.$_GET['search'].$srt.'" class="prev"><i class="fa fa-caret-left"></i></a>';
							else
								echo '<a href="' . $base_url . 'panel/admin/all-news/?page=' . ($page-1) . $srt .'" class="prev"><i class="fa fa-caret-left"></i></a>';
						}
						if($total_pages > $page) {
							if(isset($_GET['search']))
								echo '<a href="' . $base_url . 'panel/admin/all-news/?page=' . ($page+1) . '&search='.$_GET['search'].$srt.'" class="next"><i class="fa fa-caret-right"></i></a>';
							else
								echo '<a href="' . $base_url . 'panel/admin/all-news/?page=' . ($page+1) . $srt .'" class="next"><i class="fa fa-caret-right"></i></a>';
						}
						?>
					</div>
				</div>
