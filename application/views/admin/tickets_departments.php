				...
				<div class="cont clearfix">
					<div class="head clearfix">
						...
						<div class="pull-right">
							...
							<div class="dropdwn clearfix" name="dropdwn-new-department">
								<form method="post" action="<?php echo $base_url; ?>panel/admin/ticket-departments/new-department" name="new-department">
					...
					<table class="table t-departments">
						<thead>
							...
							foreach($sorting as $sorted) {
								...
								if($sort == $sorted['c']) {
									...
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/admin/ticket-departments/?sort='.$sorted['c'].'&w='.$direction.$s.'">';
									echo '<i class="fa fa-sort hid"></i>'.$arrow.$sorted['title'];
									echo '</th>';
								}else{
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/admin/ticket-departments/?sort='.$sorted['c'].'&w=d'.$s.'">';
						...
						<tbody>
							...
							?>
							<tr data-href="<?php echo $base_url . 'panel/admin/ticket-department/' . $row->id; ?>">
								...
								<td>
									...
									// Set as default
									if($row->default == '2')
										echo '<a href="'.$self . 'ticket-department/' . $row->id . '/delete'.'" title="Delete department" name="delete-department"><i class="fa fa-close"></i></a>';
									?>
									<a href="<?php echo $self . 'ticket-department/' . $row->id . '/edit'; ?>" title="Edit department"><i class="fa fa-pencil"></i></a>
									<?php
									if($row->default == '2')
										echo '<a href="'.$self.'ticket-department/'.$row->id.'/default" title="Set department as default"><i class="fa fa-anchor"></i></a>';
									?>
								</td>
							</tr>
							<?php
					...
					<div id="pagination">
						...
						if($page > 1) {
							if(isset($_GET['search']))
								echo '<a href="' . $base_url . 'panel/admin/ticket-departments/?page=' . ($page-1) . '&search='.$_GET['search'].$srt.'" class="prev"><i class="fa fa-caret-left"></i></a>';
							else
								echo '<a href="' . $base_url . 'panel/admin/ticket-departments/?page=' . ($page-1) . $srt .'" class="prev"><i class="fa fa-caret-left"></i></a>';
						}
						if($total_pages > $page) {
							if(isset($_GET['search']))
								echo '<a href="' . $base_url . 'panel/admin/ticket-departments/?page=' . ($page+1) . '&search='.$_GET['search'].$srt.'" class="next"><i class="fa fa-caret-right"></i></a>';
							else
								echo '<a href="' . $base_url . 'panel/admin/ticket-departments/?page=' . ($page+1) . $srt .'" class="next"><i class="fa fa-caret-right"></i></a>';
						}
						?>
					</div>
	...
	<script type="text/javascript">
		$('document').ready(function() {
			...
			$('a[name=delete-department]').click(function(evt) {
				var c = confirm("Are you sure you want to delete this department? All this department's tickets will be deleted!");
				if(c == false) {
					evt.preventDefault();
					return false;
				}
