...
<body>
	<div id="container" class="create-bug-report">
		...
		<div id="central-container" class="clearfix">
			<h3 class="center">CREATE BUG REPORT</h3>
			
			<?php
			if(form_error('email') == '<p>1</p>')
				echo '<div id="error" style="display:block;">Please insert a valid email address</div>';
			else
			...
			<?php
			if($allow_files == true)
				echo '<form method="POST" enctype="multipart/form-data" name="new-bug-report" action="">';
			else
				echo '<form method="POST" name="new-bug-report" action="">';
			?>
				...
				<?php
				if(form_error('email') == '<p>1</p>')
					echo '<input type="text" name="email" class="error" id="email" placeholder="Type your email..." value="'.$this->input->post('email').'" />';
				else
					echo '<input type="text" name="email" id="email" placeholder="Type your email..." value="'.$this->input->post('email').'" />';
				?>
				
				<label for="subject">SUBJECT*</label>
				<input type="text" name="subject" id="subject" placeholder="Bug report subject..." value="<?php echo $this->input->post('subject'); ?>" />
				...
				<label for="message">BUG DETAILS, PLEASE BE AS EXPLICIT AS POSSIBLE*</label>
				...
				<input type="submit" name="submit" class="pull-right" value="SUBMIT BUG REPORT" />
			</form>
		</div>
	</div>
