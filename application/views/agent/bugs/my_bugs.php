			...
			<div class="col margin-top col-sm-12">
				<div class="cont clearfix">
					<div class="head clearfix">
						<h4 class="pull-left">Bugs that I'm responsible of</h4>
						...
						<thead>
							...
							foreach($sorting as $sorted) {
								...
								if($sort == $sorted['c']) {
									...
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/my-bugs/?sort='.$sorted['c'].'&w='.$direction.$s.'">';
									echo '<i class="fa fa-sort hid"></i>'.$arrow.$sorted['title'];
									echo '</th>';
								}else{
									echo '<th width="'.$sorted['width'].'" data-sort="'.$base_url . 'panel/my-bugs/?sort='.$sorted['c'].'&w=d'.$s.'">';
						...
						<tbody>
							...
							<tr data-href="<?php echo $base_url . 'panel/bug/' . $row->access; ?>">
								...
								<td>
									<?php
									if($row->status == '1')
										echo '<div class="badge success">RECEIVED</div>';
									elseif($row->status == '2')
										echo '<div class="badge warning">TAKEN</div>';
									elseif($row->status == '3')
										echo '<div class="badge success">SOLVED</div>';
									elseif($row->status == '4')
										echo '<div class="badge primary">REVIEWED</div>';
									elseif($row->status == '5')
										echo '<div class="badge danger">INSOLVABLE</div>';
									elseif($row->status == '6')
										echo '<div class="badge primary">OTHER</div>';
									?>
								</td>
								<td>
