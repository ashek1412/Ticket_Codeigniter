...
class Loginactions_model extends CI_Model {
    ...
    public function validate_expiry($user) {
        $user = $this->db->escape($user);

        $query = $this->db->query("SELECT role, exp_date FROM tickerr_users WHERE `username`=$user");
        if($query->num_rows() == 0)
            return false;
        ...
        if($row->role == '1')
        {
            if( date('Y-m-d',strtotime($row->exp_date)) < date("Y-m-d"))
            {
