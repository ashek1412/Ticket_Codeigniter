...
class Admin_model extends CI_Model {
	...
	*/
    ...
    public function count_new_ecufiles() {
		$sql = "SELECT COUNT(*) as c FROM `tickerr_ecu_files` WHERE `file_stat`=1";
		$query = $this->db->query($sql);
		$row = $query->row();
		return $row->c;
	}
	...
	*/
	...
	public function count_all_users() {
	 ...
	 public function get_all_clients() {
            ...
            $sql = "SELECT * FROM tickerr_users where `role`=1 ORDER BY `id`";
            $query = $this->db->query($sql);
              return $query->result();
       // return $query;
    }
	...
	public function get_customers_emails() {
        //id = (int)$id;
        $sql = "SELECT * FROM tickerr_customers ";
        $query = $this->db->query($sql);
        return $query->result();;
    }
	...
	public function get_all_emails() {
        //id = (int)$id;
        $sql = "SELECT * FROM tickerr_emails ";
        $query = $this->db->query($sql);
        return $query->result();;
    }
	/**************** ENDS BLOCK OF 3 FUNCTIONS ****************/
	...
	*/
	...
	// Edit user's details
	public function edit_user($id, $name, $username, $email,$expdate) {
		$sql = "UPDATE tickerr_users SET name=?, username=?, email=?, exp_date=? WHERE id=$id";
		$this->db->query($sql, array($name, $username, $email,$expdate));
		return true;
	}
	...
	// Create new user
	public function create_user($name, $username, $email, $password, $role, $e_confirmation,$exp_date, $userid) {
    ...
    public function get_client_serial_numbers($uid) {
        $sql = "SELECT s.id as sid, s.serial,p.name FROM tickerr_serial_numbers s inner join tickerr_products p on s.productid=p.id  WHERE s.userid=? order by p.name";
    ...
    public function get_all_products() {
        $sql = "SELECT id,name FROM tickerr_products order by name";
        $query = $this->db->query($sql);
        // $row = $query->row();
        return $query;
    }

    public function add_client_serial_numbers($uid,$pid,$serial) {
        $serials = array(
            '`productid`' => $pid,
            '`userid`' => $uid,
            '`serial`' => $serial,
        ...
        // Create user and return ID
        if($this->db->insert('tickerr_serial_numbers', $serials) == true) {
            // save confirmation and return user id
    ...
    // Delete client from database
    public function delete_client_serial_numbers($id) {
        $query = $this->db->query("DELETE FROM tickerr_serial_numbers WHERE id = $id");
        if($query == true) {
	...
	public function add_news_emails($eid,$nid) {
        $serials = array(
            '`email_id`' => $eid,
            '`news_id`' => $nid,
            '`sent_stat`' => '0'
        ...
        // Create user and return ID
        if($this->db->insert('tickerr_news_emails', $serials) == true) {
            // save confirmation and return user id
    ...
    public function get_news_email_count($nid) {
         $sql = "SELECT count(te.email_id) as cnt FROM tickerr_news_emails te.news_id=? ";
