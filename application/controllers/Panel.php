...
class Panel extends CI_Controller {
	...
	public function index() {
		...
		if($config['user_info']->role == '2' || $config['user_info']->role == '3') {
			...
			// Get the site title for the header
			$config['site_title'] = $this->settings_model->get_setting('site_title');
            $config['all_news'] = $this->news_model->get_all_news();
            $config['all_news_count'] = $this->news_model->count_all_news_exp($userid);

			$this->load->view('panel_header', $config);			// Load header
			$this->load->view('panel_sidebar', $config);		// Load sidebar
			$this->load->view('client/all_news', $config);		// Load rest of the page
		}
	}
			...
			if($insert == false) {
				...
				if($this->settings_model->get_setting('mailing') == '1') {
					...
					if($this->settings_model->get_setting('send_agents_email_ticket_client_submitted') == '1') {
						...
						$this->email->clear();
                        $this->email->from($email_info['email_from_address'], $email_info['email_from_name']);
                        $this->email->subject("Thank you for your Ticket");
                        $this->email->to($userinfo->email);
						$this->email->attach($dir);
						$cid = $this->email->attachment_cid($dir);
                        $this->email->message("Thank you for choosing VZ best tuning files, we have a lot of files request now, we will start work on your file, please wait for email. <br /> <br />
                                               <span style='color:red;font-size:16px;'> Best regards from VZ-Performance center team </span> <br /> www.vz-performance.com <br /> info@vz-performance.com <br /> <span style='color:red;font-size:14px;'>Work time: </span> Monday to Saturday 09:00 to 18:00 <br />
    ...
    public function all_files() {
		...
		$this->users_model->insert_user_download_count($userid);
        ...
        if(isset($_GET['search']) and strlen($_GET['search'])>2)
        {
            ...
            foreach ($file_list as $value) {
                ...
                {
                    $results[$idx]['name'] = basename($pvalue);
                    $results[$idx]['path'] = $pvalue;
                    $results[$idx]['size'] =  intval(filesize($value)/1024);
                    $idx++;
                }
            }

		$url = "http://s10.z100.vip/fserver/restapi.php?qry=".$needle;
		$ch = curl_init();
		...
		curl_setopt($ch,CURLOPT_PORT, 24517);
		...
		{
              $results[$idx]['name'] = $res->name;
                    $results[$idx]['path'] = $res->url;
                    $results[$idx]['size'] =  $res->size;
					$arr=explode(":",  $res->path);
					$results[$idx]['fpath'] =  $arr[1];
                    $idx++;
		}
    ...
    public function file_download($file_name) {
        ...
        $userid = $this->users_model->get_user_id($this->session->tickerr_logged[0]);
        $config['user_info'] = $this->users_model->get_user_info($userid);
        $check_dl_limit=$this->users_model->get_user_download_count($userid);

        
        if(intval($check_dl_limit->dl_count)>20)
        {
            echo "<h1>Your Download Capacity exceeded for Today!</h1>";
            exit();
        ...
        if (file_exists ( $file )) 
        {
			...
			$this->users_model->update_user_download_count($userid);
	...
	public function account_settings($extra = false) {
        ...
        $current_user_serial_info = $this->client_model->get_client_serial_numbers($userid);
    ...
    public function all_news() {
        ...
        $config['all_news_count'] = $this->news_model->count_all_news_exp($userid);
            ...
            // Finish loading view
        $this->load->view('client/all_news', $config);
    }
    ...
    public function all_customers() {
        ...
        $config['agent_model'] = $this->agent_model;
       ...
       $agent_sort = array('create_at','name','license_num','vin_num','email','phone');
        ...
        if(isset($_GET['search'])) {
            $config['search'] = $_GET['search'];
            $config['all_customer'] = $this->customer_model->get_agent_customers_exp($userid, $records_per_page,$from,$sort,$sort_direction,$_GET['search']);
            $config['all_customer_count'] = $this->customer_model->count_search_agent_customers_exp($userid, $_GET['search']);
        }else{
            $config['search'] = false;
            $config['all_customer'] = $this->customer_model->get_agent_customers_exp($userid, $records_per_page,$from,$sort,$sort_direction);
            $config['all_customer_count'] = $this->customer_model->count_agent_customers_exp($userid);
        }
        ...
        // Finish loading view
        $this->load->view('agent/all_customers', $config);
    }
       ...
       // $this->load_admin_sidebar_stats($config);
        ...
        if(isset($_POST['name']))
        {
            ...
            if($insert == false) {
                ...
                $this->load_view_combo('panel/new_customer', $config);
                return;
            }else{
                ...
                header('Location: ' . $this->config->base_url() . 'panel/all-customers');
                die();
            }
        }
        else
        {
            $this->load->view('agent/new_customer', $config);
        }
    }

	// Page: panel/view-customer
    // Displays the form to View Customer
    public function view_customer($id) {
        ...
        if (isset($id)) {
            ...
            $this->load->view('agent/view_customer', $config);


        }
        else
        {
            $this->load_view_combo('agent/all_customers', $config);
        }
    ...
    public function edit_customer($id) {
        ...
        if (isset($id)) {
            ...
            $this->load->view('agent/edit_customer', $config);
    ...
    public function edit_customer_action($id) {
        ...
        if(isset($_POST['name'])) {
            ...
            $this->load_view_combo('agent/edit_customer', $config);
        }
    ...
    public function lswitch($language = "") {
