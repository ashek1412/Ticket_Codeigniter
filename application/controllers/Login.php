...
;
		if($this->loginactions_model->validate_session($user, $pass) == true) {
            ...
            if($this->loginactions_model->validate_expiry($user)==false)
                die('-1');
	...
	public function password_recovery() {
		...
		$this->load->model('Settings_model', 'settings_model', true);
        $this->load->model('Users_model', 'user_model', true);
		...
		if($this->input->post('email') != null && $this->session->password_rec_sent == null) {
			...
			$username =$this->user_model->get_user_info_email($email);
			...
			$this->email->from('info@news-vzperformance.com', 'info@news-vzperformance.com');
			$this->email->to($email);
